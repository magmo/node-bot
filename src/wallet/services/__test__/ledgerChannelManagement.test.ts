...
describe('openLedgerChannel', () => {
    ...
    it('should return an allocator channel and a signed commitment', async () => {
        const {
            commitment,
            signature,
        } = await LedgerChannelManager.openLedgerChannel(
            pre_fund_setup_0,
            theirSignature,
            appAttributesFromBytes,
        );
        expect(commitment).toMatchObject(pre_fund_setup_1_response);
        expect(ChannelManagement.validSignature(commitment, signature)).toBe(
            true,
        );
    });
...
describe('updateLedgerChannel', () => {
    describe('transitioning to a postFundSetup commitment', () => {
        ...
        it('should return an allocator channel and a signed commitment', async () => {
            ...
            expect(
                ChannelManagement.validSignature(commitment, signature),
            ).toBe(true);
        });
...
describe('nextCommitment', () => {
    it('works on preFundSetup commitments', () => {
        expect(
            LedgerChannelManager.nextCommitment(pre_fund_setup_0),
        ).toMatchObject(pre_fund_setup_1);
    });

    it('works on postFundSetup commitments', () => {
        expect(
            LedgerChannelManager.nextCommitment(post_fund_setup_0),
        ).toMatchObject(post_fund_setup_1);
    });
