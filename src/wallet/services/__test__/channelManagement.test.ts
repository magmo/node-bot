...
describe('validSignature', () => {
  it('returns true when the commitment was signed by the mover', async () => {
    expect(
      ChannelManagement.validSignature(
        asCoreCommitment(pre_fund_setup_1),
        hubSignature,
      ),
    ).toBe(true);
  });
  ...
  it('returns false when the commitment was not signed by the mover', async () => {
    expect(
      ChannelManagement.validSignature(
        asCoreCommitment(pre_fund_setup_0),
        hubSignature,
      ),
    ).toBe(false);
  });
...
describe('formResponse', () => {
  it('returns a signed core commitment', async () => {
    ...
    expect(
      await ChannelManagement.formResponse(channel.id, bytesFromAppAttributes),
    ).toMatchObject({
...
describe('nextCommitment', () => {
  it('works on preFundSetup commitments', () => {
    expect(ChannelManagement.nextCommitment(pre_fund_setup_0)).toMatchObject(
      pre_fund_setup_1,
    );
  });

  it('works on postFundSetup commitments', () => {
    expect(ChannelManagement.nextCommitment(post_fund_setup_0)).toMatchObject(
      post_fund_setup_1,
    );
  });

  it('throws on app commitments', () => {
    expect(() => ChannelManagement.nextCommitment(app_0)).toThrowError(
      'CommitmentType.App',
    );
  });
